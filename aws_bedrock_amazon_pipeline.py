"""
title: AWS Bedrock Amazon Pipeline
author: redhawkeye
date: 2025-01-20
version: 1.0
license: MIT
description: A pipeline for generating text and processing images using the AWS Bedrock API(By Amazon).
...
class Pipeline:
    ...
    def get_models(self):
        if self.valves.AWS_ACCESS_KEY and self.valves.AWS_SECRET_KEY:
            try:
                response = self.bedrock.list_foundation_models(byProvider='Amazon', byInferenceType='ON_DEMAND')
                return [
                    {
                        "id": model["modelId"],
